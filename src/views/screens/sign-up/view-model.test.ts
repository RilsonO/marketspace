...
describe('SignUp view-model [useSignUpViewModel]', () => {
  ...
  it('should show an error toast on handleUserPhotoSelect in case of AppError', async () => {
    const launchImageLibraryAsyncMock = jest.spyOn(
      ImagePicker,
      'launchImageLibraryAsync'
    );
    const mockAppError = new AppError('AppError message');
    launchImageLibraryAsyncMock.mockRejectedValueOnce(mockAppError);
  ...
  it(`should show an AppError error toast on handleSignUp when throw exception 
      caused by signUp`, async () => {
    const mockAppError = new AppError('AppError message');
    const signUpMock = jest.spyOn(userRepository, 'signUp');
    signUpMock.mockRejectedValueOnce(mockAppError);
  ...
  it(`should show an generic error toast on handleSignUp when throw exception 
    ...
    const signInUserMock = jest.spyOn(useAuthViewModel(), 'signInUser');
